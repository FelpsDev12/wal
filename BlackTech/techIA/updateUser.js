...
async function getUserData() {
  ...
  try {
    ...
    <p id='toolsEmail'>${data.email}</p>
    <p id='toolsRole'>${data.role}</p>
    <p id='toolsAvisos'>${data.warn}</p>
    <p id='toolsTotalBan'>${data.totalban}</p>
    <p id='toolsTokens'>Você gerou ${data.warn} novos tokens</p>
    `
  } catch (error) {
    console.error(error)
  ...
  const username = inputUpdateUsername.value.trim();
   ...
   async function updateEmail() {
    const token = localStorage.getItem('token')
    const email = document.getElementById('inputUpdateEmail').value.trim()
    ...
    if (!res.ok) {
      alert('Falha na atualizaçao de email', data.error)
      return;
    }
    ...
    console.error(error)
    alert('Erro de comunicação no front')
    }
   };
   ...
   async function updatePassword() {
    ...
    if (!res.ok) {
      alert('Erro ao atualizar senha do usuario')
      return;
    }
...
async function deleteUser() {
  ...
  if (!res.ok) {
    return alert('erro ao delete usuario front');
  }
  ...
  console.error(error)
  alert('erro de comunicacao no front')
}
}
setInterval(() => {
  getUserData()
